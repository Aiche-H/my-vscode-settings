# my extensions

## liveserver

![Live Server](./img/liveserver.png)

## AI

![Qodo AI](./img/qodo.png)
![Codeium](./img/AIassist.png)

## Intellisense

![NPM Intellisense](./img/npmintel.png)
![Path Intellisense](./img/pathintel.png)
![Intellicode](./img/itelli.png)
![CSS Intellisense](./img/HTMLCSS.png)
![Class name Intelllisense](./img/classintel.png)
![API Intelllisense](./img/APIintel.png)

## JSON

![Paste JSON as code](./img/jsonascode.png)
![Sort JSON](./img/sortJSON.png)
![JSON crack](./img/crack.png)

## Exports

![Exports autocoplete](./img/exportauto.png)
![Find unused exports](./img/unusedexport.png)

## JS

![Babel JavaScript](./img/babel.png)
![Prettier - Code formatter](./img/prettier.png)
![ES6-snippet](./img/ES6-snippet.png)
![Prettier - Code formatter](./img/JS-boost.png)

## Git

![gitignore](./img/ignore.png)
![github pull requests](./img/pull.png)
![Git graph](./img/graph.png)
![Git history](./img/history.png)
![Gitlense](./img/lens.png)

## Viewers

![Docx viewer](./img/docview.png)
![Pdf viewer](./img/pdfview.png)
![Excel viewer](./img/excel.png)

## Markdown

![Markdown Navigation](./img/mdnav.png)
![markdownlint](./img/mdlint.png)

## HTML

![HTML end tag lables](./img/endtag-lable.png)
![AutoRenameTag](./img/AutoRenameTag.png)

## env

![dotenv support](./img/dotenv.png)

## helpful

![Turbo console log](./img/clog.png)
![Find it faster](./img/fif.png)
![CSS peek](./img/peek.png)
![Code runner](./img/runner.png)
![TODO tree](./img/Bettercomments.png)
![comment-anchor](./img/comment-anchor.png)
![change-case](./img/change-case.png)
![TabOut](./img/TabOut.png)
![Error lense](./img/error.png)
![Live share](./img/share.png)
![Search by Sourcegraph](./img/sourcegraph.png)
![Draw.io integration](./img/drawio.png)

## visuals

![Custom CSS and JS loader](./img/customcss.png)
![Material icons theme](./img/icons.png)
![Atom One Dark Theme](./img/theme.png)
